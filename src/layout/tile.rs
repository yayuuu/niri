...
impl<W: LayoutElement> Tile<W> {
    ...
    fn render_inner<R: NiriRenderer>(
        ...
        if self.focused_window().sizing_mode() == SizingMode::Normal {
            self.tab_indicator
                .render(ctx.renderer, tab_indicator_loc, &mut |elem| push(elem.into()));
        }
        ...
        if self.background_effect.is_visible() {
            ...
            let blur_geometry = if let Some(rects) = self.window.focused_window().blur_region() {
                if rects.is_empty() {
